(function () {
  ...
  /* ---------------- Formulário de contato -> também pode ser enviado por e-mail ---------------- */
  var emailLink = document.getElementById("cf-email-link");
  if (emailLink && form) {
    emailLink.addEventListener("click", function (e) {
      ...
      var bodyLines = ["Olá, meu nome é " + nome + "."];
      if (empresa) bodyLines.push("Empresa: " + empresa + ".");
      if (email) bodyLines.push("Meu e-mail para retorno: " + email + ".");
      if (telefone) bodyLines.push("Telefone/WhatsApp: " + telefone + ".");
      if (objetivo) bodyLines.push("O que busco: " + objetivo + ".");
      if (mensagem) bodyLines.push("Mensagem: " + mensagem);

      var subject = encodeURIComponent("Contato pelo site — " + nome + (empresa ? " (" + empresa + ")" : ""));
      var body = encodeURIComponent(bodyLines.join("\n"));
      emailLink.setAttribute("href", "mailto:contato@saluttiempresarial.com.br?subject=" + subject + "&body=" + body);
    });
  }
  ...
  if (depoCarousel) {
    ...
    function depoUpdate() {
      ...
      if (depoPrevBtn) {
        ...
        depoPrevBtn.disabled = depoIndex <= 0;
      }
      if (depoNextBtn) {
        depoNextBtn.style.display = hasOverflow ? "" : "none";
        depoNextBtn.disabled = depoIndex >= maxIndex;
      }
    }

    if (depoPrevBtn) {
      depoPrevBtn.addEventListener("click", function () {
        depoIndex = Math.max(0, depoIndex - 1);
        depoUpdate();
      });
    }
    if (depoNextBtn) {
      depoNextBtn.addEventListener("click", function () {
        ...
        depoIndex = Math.min(maxIndex, depoIndex + 1);
        depoUpdate();
      });
    }
